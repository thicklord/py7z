...
# noinspection DuplicatedCode
def new_walker(path_obj, exts, archive_list):
	arc_count = 0
	...
	for root, drz, flz in os.walk(path_obj, topdown=True):
		...
		for fl in flz:
			...
			if fext in archive_types:
				...
				walker(new_dir, exts, archive_list)
...
# noinspection DuplicatedCode
def master_blaster(some_path, extensions):
	...
	while True:
		breaker = new_walker(new_root, extensions, master_archives_set)
	...
	for ap in master_archives_set:
		if piff(ap) and extensions.remove:
			send2trash(ap)
